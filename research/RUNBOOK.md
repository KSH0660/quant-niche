...
운영자 결정(2026-06 개정): **자동화 우선**. 과거 "수동 우선" 방침은 폐기. 아래 체인은 **`/quant-daily` 커맨드**(살아있는 로컬 세션이 서브에이전트를 순차 소환)로 오케스트레이션되고, **`/loop /quant-daily`**로 주기 실행된다(별도 헤드리스 CLI·OS crontab 불필요). 자동화 범위·실거래 게이트는 `CLAUDE.md` §운영 모델을 따른다(판정까지 자동 + Opus 투입, 실거래 집행은 `HUMAN_APPROVAL` 게이트 — 운영자가 토글).
...
각 에이전트는 대상 `ideas/` 파일을 읽고 자기 섹션만 채운 뒤 다음으로 넘긴다(이 배턴 넘기기를 `/quant-daily` 세션이 자동 전달). `rejected`도 파일·INDEX에 보존(기각 논리·다음 단서 필수). 6단계 판정은 **가장 똑똑한 모델(Opus)**이 맡고, 결과는 최상단 브리프(`data/brief/`)에 권고로 적재된다 — 실거래 주문 전송만 게이트를 거친다.
...
- **실행 형태**: 운영자가 로컬에 Claude Code 세션을 켜두고 `/loop /quant-daily`. 세션이 살아있는 동안 주기적으로 자기 실행(노트북 깨어있을 때만 — 닫으면 멈춤. 이게 OS crontab/원격 대신 택한 트레이드오프).
- **0단계 수집**: `/quant-daily`가 `make collect`(코드·$0)로 ledger 갱신 → 신규/변경 actionable 이벤트 파악.
- **이벤트 트리거**: actionable 이벤트가 있을 때만 세션이 에이전트 체인을 직접 소환(발굴→정찰→레드팀→검증→마찰→Opus 판정). 새 게 없으면 싸게 종료(idempotency).
- **브리프 갱신**: 판정 결과를 `data/brief/BRIEF.md`의 "지금 기회 + 최종 행동 조언"으로 랭킹·적재. 운영자는 `make brief`로 이것만 본다.

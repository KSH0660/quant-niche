...
운영자 결정(2026-06 개정): **자동화 우선**. 과거 "수동 우선" 방침은 폐기. 아래 체인은 **일일 cron 태스크**(`scripts/daily.sh` → 헤드리스 Claude가 `scripts/daily-prompt.md` 지시대로 서브에이전트를 순차 소환)로 자동 오케스트레이션되고 **매일** 백그라운드에서 돈다. 자동화 범위·실거래 게이트는 `CLAUDE.md` §운영 모델을 따른다(판정까지 자동 + Opus 투입, 실거래 집행은 `HUMAN_APPROVAL` 게이트 — 운영자가 토글).
...
각 에이전트는 대상 `ideas/` 파일을 읽고 자기 섹션만 채운 뒤 다음으로 넘긴다(이 배턴 넘기기를 일일 cron 태스크가 자동 전달). `rejected`도 파일·INDEX에 보존(기각 논리·다음 단서 필수). 6단계 판정은 **가장 똑똑한 모델(Opus)**이 맡고, 결과는 최상단 브리프(`data/brief/`)에 권고로 적재된다 — 실거래 주문 전송만 게이트를 거친다.
...
- **이벤트 트리거**: actionable 이벤트마다 헤드리스 Claude가 에이전트 체인을 직접 소환(발굴→정찰→레드팀→검증→마찰→Opus 판정).
